...
public class Catan {
	 ...
	 */
	public Catan(ArrayList<Player> players, boolean normalTokenSequence){
		this.island = new Island(this, normalTokenSequence);
	...
	public void changeTurnTaker(){
		...
		if(turnInRound < players.size()){
			if(round == Settings.SECOND_FOUNDATION_PHASE){
	 ...
	 * (In case players had the same dice result, the player who rolled the dice first
	 * gets in place before the other one)
	 * @param diceResult Results of both dices added
	 */
	public void saveDiceResultAndChangeOrder(double diceResult) {
		//Check whether another player had same dice result
		for(int previousResults = 0; previousResults < diceResultsOrder.size(); previousResults++){
			if(diceResultsOrder.get(previousResults) == diceResult){
				diceResult = diceResult - 0.1;
			}
		}
		diceResultsOrder.add(diceResult);
		
		//If everyone diced, bring players in right order
		if(turnInRound == players.size()-1){
			//HashMap with result of dicing and the belonging player
			HashMap<Double, PlayerClient> hashMap = new HashMap<Double, PlayerClient>();
			for(int player = 0; player < players.size(); player++){
				hashMap.put(diceResultsOrder.get(player), (PlayerClient)players.get(player));
			}
			Collections.sort(diceResultsOrder);
			players.clear();
			//Add players in right order
			for(int player = diceResultsOrder.size()-1; player >= 0 ; player--){
				players.add(hashMap.get(diceResultsOrder.get(player)));
			}   
		}
	}
	...
	public boolean checkLargestArmy() {
		...
		if(playerWithLA != null && playerWithLA.getPlayerID() != playerWithLargestArmy){
			if(playerWithLargestArmy != -1){
				((PlayerClient)getPlayerWithID(playerWithLargestArmy)).
				      addAndCheckVictoryPoints(-(Settings.VICTORY_POINTS_FOR_LARGEST_ARMY));
			}
			playerWithLA.addAndCheckVictoryPoints(Settings.VICTORY_POINTS_FOR_LARGEST_ARMY);
	...
	public LongestRoadChanges checkLongestRoad() {
		...
		if(playerWithLR == null || multiplePlayersWithLR > 1){
			if(playerWithLongestRoad != -1){
				((PlayerClient)getPlayerWithID(playerWithLongestRoad)).
				         addAndCheckVictoryPoints(-(Settings.VICTORY_POINTS_FOR_LONGEST_ROAD));
				playerWithLongestRoad = -1;
				return LongestRoadChanges.NO_ONE;
			}
			return LongestRoadChanges.SAME_OWNER;
		}
		...
		// four roads.
			...
			playerWithLR.addAndCheckVictoryPoints(Settings.VICTORY_POINTS_FOR_LONGEST_ROAD);
	...
	/**
	 * @return the ID of the playerWithLargestArmy
	 */
	...
	/**
	 * @return the ID of the playerWithLongestRoad
	 */
