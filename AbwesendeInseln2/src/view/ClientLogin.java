...
public class ClientLogin extends AnchorPane{
	...
	private void addElements() {
		...
		dragArea.setPrefHeight(Settings.WINDOW_BAR_HEIGHT);
		dragArea.setPrefWidth(Settings.WINDOW_BAR_WIDTH);
		...
		RadioButton normalTokenSequenceButton = new RadioButton("Play with a random token order");
		normalTokenSequenceButton.setFont(Font.loadFont(Settings.OSWALD_PATH, Settings.BIG_FONT_SIZE));
		normalTokenSequenceButton.setTextFill(Color.WHITE);
		
		GridPane serverStart = new GridPane();
		serverStart.add(orServer, 0, 0);
		serverStart.add(serverBox, 0, 1);
		serverStart.add(normalTokenSequenceButton, 0, 2, 2, 1);
		serverStart.add(myHostIP, 1, 1);
		serverStart.add(startServer, 0, 5);
		serverStart.add(logLevel, 1, 5);
		...
		startServer.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event) {
				try{
					...
					boolean normalTokenSequence = !normalTokenSequenceButton.isSelected();
					new ServerControl(port, normalTokenSequence);
				...
				normalTokenSequenceButton.setDisable(true);
			}
		});
