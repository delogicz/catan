...
/**
 * Creates the skin of a TradeOffer - object
...
public class TradeOfferSkin extends GridPane{
	 ...
	 * @param tradeOffer TradeOffer that is turned into a view
	 */
	public TradeOfferSkin(Player player, TradeOffer tradeOffer){
		...
		case RED: 		setStyle("-fx-background-color: SALMON; -fx-padding: 10;"); break;
		case ORANGE: 	setStyle("-fx-background-color: ORANGE; -fx-padding: 10;"); break;
		case BLUE: 		setStyle("-fx-background-color: DEEPSKYBLUE; -fx-padding: 10;"); break;
		...
		if(player.getPlayerID() != player.getCatan().getPlayerClient().getPlayerID()){
			...
			enterButton.setOnAction(new EventHandler<ActionEvent>(){
				...
				@Override
				public void handle(ActionEvent event) {
					if(player.getCatan().getTrade(tradeOffer.getTradeId()) != null){
		...
		Button deleteTradeButton = new Button("Delete");
