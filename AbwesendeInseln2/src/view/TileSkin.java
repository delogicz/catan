...
public class TileSkin  {
	...
	public TileSkin(final Tile hex) {
		...
		try {
			...
			hexagon.getPoints().addAll(new Double[] { 0d, 1d, Settings.HALF_SQRT_THREE, 0.5, Settings.HALF_SQRT_THREE, -0.5, 0d, -1d,
					-Settings.HALF_SQRT_THREE, -0.5, -Settings.HALF_SQRT_THREE, 0.5 });
			...
			hexagon.setTranslateX(2 * Settings.HALF_SQRT_THREE * Settings.SCALE * hex.getPos()
					+ Settings.HALF_SQRT_THREE * Settings.SCALE * Math.abs(hex.getRow() - 2) + Settings.OFFSET_X);
			...
			hexagon.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
				...
				public void handle(MouseEvent mouseEvent) {
					if(hex.getCatan().getPlayerClient().getRelocateRobber() && hex != hex.getCatan().getIsland().getRobber().getTile()){
						hex.relocateRobber();
					}
				}
			});
