...
public class PlayerClient extends Player {
	...
	private boolean foundationPhase;
	
	
	/**
	 * Constructor, creates a PlayerClient, all Players for the server are PlayerClient, 
	 * for the Clients only the Client it self is a PlayerClient all other Players are Players
	 * @param playerColor
	 * @param playerId
	 * @param name
	 * @param status
			...
			public void run() {
				...
				switch(status){
					case Settings.DORF_BAUEN: playerControl.buildSettlement();
					                          ...
					                          playerControl.getSkin().setTurnText("Your");
					                          break;
					...
					case Settings.HANDELN_BAUEN: 	playerControl.enableAllButtons();
													break;
													
					case Settings.WARTEN: if(previousStatus.equals(Settings.WUERFELN) || 
											  previousStatus.equals(Settings.KARTEN_ABGEBEN) ){
													catan.getGameManager().getApp().closeAllPopUpStages();
											}
				}
			}catch(Exception e){
	...
	public Building build(String typ, Vertex vertex, boolean server) {
		...
		if(typ.equals(Settings.DORF)){
			...
			if(foundationPhase && settlements.size()==2) foundationPhase = false;
	 ...
	 * @param pointsToBeAdded the amount of victory points the player receives
	 * @return true if game is won
	 */
	public boolean addAndCheckVictoryPoints(int pointsToBeAdded){
		this.victoryPoints += pointsToBeAdded;
		if(victoryPoints >= Settings.VICTORY_POINTS_NEEDED_TO_WIN 
				|| victoryPoints + devVictoryPoints >= Settings.VICTORY_POINTS_NEEDED_TO_WIN){
			this.victoryPoints += devVictoryPoints;
			return true;
		}
		return false;
	}
	...
	public boolean canBuildSettlement(Vertex vertex) {
		...
		if(settlements.size() < 2 && foundationPhase){
	...
	public boolean canBuildRoad(Edge edge) {
		...
		if(!edge.getVertexFrom().isOccupied()){
		...
		if(!edge.getVertexTo().isOccupied()){
	...
	/**
	 * Creates playerControl for the player
	 */
	...
	public void setRelocateRobber(boolean relocateRobber) {
		...
		if(relocateRobber){
			catan.getGameSkin().setCursor(Cursor.HAND);
		}
